...
var sum = function(array, product=0) {
  //base
  if(array.length === 0){
    return product;
  }
  //recursion
  product += array[0];
  return sum(array.slice(1), product)
...
// sumBelow(10); // 45
// sumBelow(7); // 21
var sumBelow = function(n, product=0) {
  //base
if(n === 1 || n === -1){
  return product;
}else if(n === 0){
...
var palindrome = function(string) {
  let x = string.replace(/[\s]/g,'').toLowerCase();
  console.log(x);
  // base
  if(x.length === 1){
    return true;
  ...
  // recursion
  return palindrome(x.slice(1, x.length-1))
